...
export default function optionsSanitiser(options: RegistryOptions): RegistryConfig {
  ...
  if (typeof fallbackRegistryUrl !== 'undefined' && !fallbackRegistryUrl.endsWith('/')) {
    fallbackRegistryUrl += '/';
  }
  ...
  return {
    ...
    tempDir: options.tempDir ?? settings.registry.defaultTempPath,
    verbosity: options.verbosity ?? 0,
    discovery: typeof options.discovery === 'boolean' ? options.discovery : true,
    pollingInterval: typeof options.pollingInterval !== 'undefined' ? options.pollingInterval : 5,
    env: options.env ?? {},
