...
async function buildClient({
  ...
  await Deno.writeTextFile(
    wrapperFilePath,
    reactWrapper(join(componentPath, entrypoint))
  );
  ...
  const bundle = await esbuild.build({
    ...
    outfile: '',
    platform: 'neutral',
    plugins: [...denoPlugins({ importMapURL: await getImportMapURL() })],
...
async function wrapClient(
  ...
  await Promise.all([
    Deno.writeTextFile(
      join(packageDir, 'template.js'),
      viewWrapper(templateHash, templateString)
    ),
...
export async function compile(
  ...
  const cleanTempFiles = () =>
    Promise.all([
      tryRemove(clientWrapperFilePath),
      tryRemove(serverWrapperFilePath),
    ]);
  ...
  try {
    ...
    await Promise.all([
      wrapClient(client, packageDir),
      wrapServer(server, packageDir),
    ]);
